...
def solve():
    N, M = read_ints()
    paint = [i if i < N else i - 1 for i in read_ints()]
    paint.sort()
    pfs = list(accumulate(paint, initial=0))
    tot = pfs[-1]
    ans = 0
    r = bisect_left(paint, N - paint[0])
    for l in range(M - 1):
        v = paint[l]
        r = max(r, l + 1)
        while r - 1 > l and paint[r - 1] >= N - v:
            r -= 1
        ans += (tot - pfs[r]) - (M - r) * (N - v - 1)
    ans *= 2
    write_int(ans)
