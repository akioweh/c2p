"""
https://codeforces.com/group/IiWiF2p9CH/contest/585069/problem/J
"""
...
def solve():
    ...
    # dp[j] = state for subproblem arr[:j]
    # state[set] = min pen achievable with this set of prime (factor)s used
    # we'll consider how to reconstruct the answer later~~

    dp = [
        {0: 0}
    ]
    for i in range(N):  # j = i + 1
        val = arr[i]
        prev = dp[i]
        cur = defaultdict(int)
        for mask, score in  prev.items():
